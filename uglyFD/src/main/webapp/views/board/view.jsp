...
<section id="content">
	<div id="board-write-container">
		...
		<table id="tbl-board">
			...
			<tr>
				<th colspan="2">
					<%-- 					<c:if test="${ not empty loginMember && loginMember.id == board.writerId 이부분 수정해야됨 }"> </c:if> --%>
					<button type="button" onclick="location.href='${path}/board/update?no=${board.b_no }'">수정</button>
	...
	$(document).ready(() => {
		$('#btnDelete').on('click', () => {
			if(confirm('정말로 게시글을 삭제 하시겠습니까?')) {
				location.replace('${ path }/board/delete?no=${ board.b_no }');
			}
		});	
